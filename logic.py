...
class DatabaseManager:
    ...
    def create_tables(self):
        ...
        ''')
            ...
            conn.executemany("INSERT INTO prizes (image, used) VALUES (?, 0)", data)
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('''
                SELECT users.user_name, COUNT(winners.prize_id) as prizes_won
                FROM users
...
def hide_img(img_name):
    ...
    cv2.imwrite(f'./M4L1/hidden_img/{img_name}', pixelated_image)
